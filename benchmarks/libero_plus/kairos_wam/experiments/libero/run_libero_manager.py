...
from libero_plus_eval_utils import LIBERO_PLUS_SUITES, build_suite, resolve_category_values
...
def _resolve_suite_names(task_suite_names: list[str], include_libero_mix: bool) -> tuple[list[str], bool]:
    ...
    if unknown:
        raise ValueError(
            f"Unknown suite name(s): {unknown}. "
            f"Expected one of {list(LIBERO_PLUS_SUITES) + ['libero_mix']}"
        )

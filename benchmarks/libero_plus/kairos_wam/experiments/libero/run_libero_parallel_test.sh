...
run_libero_eval() {
    ...
    resolve_simple_override() {
        local key="$1"
        local token
        # Literal prefix match (Hydra keys contain dots; avoid sed metacharacters).
        for token in ${EXTRA_ARGS:-}; do
            case "$token" in
                "${key}="*)
                    echo "$token"
    ...
    NUM_INFERENCE_STEPS_OVERRIDE="$(resolve_simple_override 'EVALUATION.num_inference_steps')"
    EVAL_NUM_INFERENCE_STEPS_OVERRIDE="$(resolve_simple_override 'eval_num_inference_steps')"
    ...
    launch_task_on_pane() {
        ...
        tmux send-keys -t $SESSION_NAME:$pane_info \
            "( unset NVIDIA_VISIBLE_DEVICES; export CUDA_VISIBLE_DEVICES=\$PHYSICAL_GPU MUJOCO_EGL_DEVICE_ID=\$PHYSICAL_GPU; echo \"[worker-env] CUDA_VISIBLE_DEVICES=\$CUDA_VISIBLE_DEVICES MUJOCO_EGL_DEVICE_ID=\$MUJOCO_EGL_DEVICE_ID MUJOCO_GL=\${MUJOCO_GL:-} PYOPENGL_PLATFORM=\${PYOPENGL_PLATFORM:-} LIBERO_EGL_DEVICE_ID_MODE=\${LIBERO_EGL_DEVICE_ID_MODE:-} __EGL_VENDOR_LIBRARY_FILENAMES=\${__EGL_VENDOR_LIBRARY_FILENAMES:-} NVIDIA_VISIBLE_DEVICES=\${NVIDIA_VISIBLE_DEVICES:-}\"; \"\$EVAL_PYTHON\" experiments/libero/eval_libero_single.py task=$CONFIG ckpt=$CKPT EVALUATION.task_suite_name=\$SUITE EVALUATION.task_id=\$TASK_ID gpu_id=${physical_gpu} EVALUATION.num_trials=$NUM_TRIALS EVALUATION.output_dir=$OUTPUT_DIR $category_arg $MODEL_ADAPTER_OVERRIDE $MODEL_ENDPOINT_OVERRIDE $DATASET_STATS_OVERRIDE $NUM_INFERENCE_STEPS_OVERRIDE $EVAL_NUM_INFERENCE_STEPS_OVERRIDE ) > \"\$LOG_FILE\" 2>&1 & pid=\$!; echo \"RUNNING|$gpu_id|\$pid|\$(date +%s)|\$LOG_FILE\" > \"\$STATUS_FILE\"; wait \$pid; rc=\$?; if [ \$rc -eq 0 ] && [ -f \"\$RESULT_FILE\" ]; then echo \"SUCCESS|$gpu_id|\$rc|\$(date +%s)|\$LOG_FILE\" > \"\$STATUS_FILE\"; else echo \"FAILED|$gpu_id|\$rc|\$(date +%s)|\$LOG_FILE\" > \"\$STATUS_FILE\"; fi" \
            C-m 2>/dev/null
        return 0
    }
    ...
    launch_task_worker() {
        ...
        if [ -n "$NUM_INFERENCE_STEPS_OVERRIDE" ]; then
            hydra_args+=("$NUM_INFERENCE_STEPS_OVERRIDE")
        fi
        if [ -n "$EVAL_NUM_INFERENCE_STEPS_OVERRIDE" ]; then
            hydra_args+=("$EVAL_NUM_INFERENCE_STEPS_OVERRIDE")
        fi

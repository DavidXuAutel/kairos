...
def main() -> None:
    ...
    parser.add_argument(
        ...
        "Explicit --suite replaces the default (does not append to it).",
    )
    parser.add_argument(
        "--include-libero-mix",
        action="store_true",
    ...
    task_suite_names = list(args.suite) if args.suite else ["libero_mix"]
    unknown = [
        s
        for s in task_suite_names
        if s not in ("libero_mix", "libero_spatial", "libero_object", "libero_goal", "libero_10")
    ]
    if unknown:
        raise SystemExit(f"Unknown --suite value(s): {unknown}")

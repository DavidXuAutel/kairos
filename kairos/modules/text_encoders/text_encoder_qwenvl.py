...
class QwenVLTextEncoder(nn.Module):
    def __init__(self, dtype=torch.bfloat16, device='cuda', from_pretrained=''):
        super().__init__()
        # Optional offload for low-VRAM hosts (e.g. RTX 4090 24GB + full Qwen-7B).
        device = os.environ.get('KAIROS_TEXT_ENCODER_DEVICE', device)
        print(f'QwenVLTextEncoder device={device}', flush=True)
